document.getElementById("yesButton").addEventListener("mouseover", function () {
    ...
    var newPositionX = Math.max(0, Math.min(window.innerWidth - dialogBox.offsetWidth, Math.floor(Math.random() * (container.offsetWidth - dialogBox.offsetWidth - 320))));
    var newPositionY = Math.max(0, Math.min(window.innerHeight - dialogBox.offsetHeight, Math.floor(Math.random() * (container.offsetHeight - dialogBox.offsetHeight - 320))));
